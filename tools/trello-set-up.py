...
if response.status_code == 200:
    TRELLO_BOARD_ID = response.json()['id']
    print(f"Board created with name \"{board_name}\" and id \"{TRELLO_BOARD_ID}\"")
else:
...
for title in lists:
    ...
    response = requests.post(url_builder(f"/boards/{TRELLO_BOARD_ID}/lists"), params=queryParams)

    if response.status_code == 200:
        print(f"List created with name \"{title}\"")


print(f"Set TRELLO_BOARD_ID in .env")
print(f"{TRELLO_BOARD_ID=}")
